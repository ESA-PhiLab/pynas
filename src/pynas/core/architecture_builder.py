...
def generate_layer_code():
    ...
    def get_random_value(param, section, is_float=False, is_odd=False, format_str=None):
        ...
        min_val = (
            config.getfloat(section, f"min_{param}")
            if is_float
            else config.getint(section, f"min_{param}")
        )
        max_val = (
            config.getfloat(section, f"max_{param}")
            if is_float
            else config.getint(section, f"max_{param}")
        )
    ...
    for param in parameters:
        ...
        if param == "activation":
            activation_code = random.choice(
                ["r", "g"]
            )  # Use only ReLU and GELU for intermediate layers
                ...
                for param_name, code in parameter_vocabulary.items():
                    if code == param_code and param_name in param_definitions:
                        if param_name == "dropout_rate":
                            ...
                            segment_info[param_name] = activation_functions_vocabulary.get(
                                params[i], "Unknown"
                            )
                            i += 1
...
def generate_code_from_parsed_architecture(parsed_layers):
    ...
    reverse_activation_functions_vocabulary = {
        v: k for k, v in activation_functions_vocabulary.items()
    }
    ...
    for layer in parsed_layers:
        ...
        for param_name, param_value in layer.items():
            ...
            if param_name in parameter_vocabulary:
                ...
                if param_name == "out_channels_coefficient":
                    segment_code += f"{param_code}{int(param_value):02d}"
                    continue
                if param_name == "dropout_rate":
                    segment_code += f"{param_code}{float(param_value):.2f}"
                    continue
                if param_name == "activation":
                    param_value = reverse_activation_functions_vocabulary.get(
