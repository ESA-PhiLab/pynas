...
def update_config_path(config_path, model_path):
    ...
    with config_path.open("r", encoding="utf-8") as f:
        config = json.load(f)
    config["model_filepath"] = str(model_path)
...
class Population:
    def __init__(self, n_individuals, max_layers, dm, max_parameters=100_000, save_directory=None):
        ...
        # File storage
        self.save_directory = Path(save_directory or "models_traced")
        self.save_directory.joinpath("src").mkdir(parents=True, exist_ok=True)
        self.save_directory.joinpath("backups").mkdir(parents=True, exist_ok=True)
    ...
    def setup_logger(log_file="logs/population.log", log_level=logging.DEBUG):
        ...
        if log_file.exists():
            ...
            log_file = Path(f"{base}_{timestamp}{ext}")
    ...
    def create_random_individual(self, max_attempts=5):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if not self.check_individual(individual):
                    self.logger.warning(
                        f"Created individual failed model validation "
        ...
        try:
            ...
            for individual in self.population:
                ...
                if (
                    ...
                    and not np.isnan(individual.fitness)
                ):
            ...
            if invalid_count > 0:
                self.logger.warning(
                    f"Found {invalid_count} individuals with invalid fitness values"
                )
            ...
            if sorted_population:
                top_fitness = [
                    ind.fitness for ind in sorted_population[: min(3, len(sorted_population))]
                ]
    ...
    def _checkpoint(self):
        ...
        try:
            ...
            backup_dir = self.save_directory / "backups" / f"gen_{self.generation}"
            backup_dir.mkdir(parents=True, exist_ok=True)

            # Backup previous files if they exist
            for file_type in ["population", "df_population"]:
                src_path = self.save_directory / "src" / f"{file_type}_{self.generation}.pkl"
                if src_path.exists():
                    backup_path = (
                        backup_dir
                        / f"{file_type}_{self.generation}_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.pkl"
                    )
                    try:
            ...
            if save_success:
                self.logger.info(
                    f"Successfully checkpointed population at generation {self.generation}"
                )
                return True
            else:
    ...
    def check_individual(self, individual):
        ...
        try:
            ...
            self.logger.debug(
                f"Building model from individual with {len(individual.parsed_layers)} layers"
            )
            model_representation, is_valid = self.build_model(individual.parsed_layers)

            if not is_valid:
                self.logger.warning(
                    f"Model building failed for individual: build_model returned is_valid=False"
                )
                return False
            ...
            if modelSize >= self.max_parameters:
                self.logger.warning(
                    f"Model too large: {modelSize} parameters (max: {self.max_parameters})"
                )
                return False
    ...
    def create_population(self, max_attempts=200, timeout_seconds=300):
        ...
        if len(population) < self.n_individuals:
            ...
            with tqdm.tqdm(
                total=self.n_individuals - len(population), desc="Filling Missing"
            ) as pbar:
                ...
                while len(population) < self.n_individuals:
                    ...
                    if (
                        time.time() - fill_start_time > timeout_seconds / 2
                    ):  # Allow half the original timeout
                        self.logger.warning(
                            "Timed out while trying to fill population after duplicate removal"
                        )
                        break

                    if (
                        additional_attempts >= max_attempts / 2
                    ):  # Allow half the original max attempts
                        self.logger.warning(
                            "Reached maximum attempts while trying to fill population after duplicate removal"
                        )
                        break
                    ...
                    try:
                        ...
                        existing_archs = set(
                            getattr(ind, "architecture", str(ind.parsed_layers))
        ...
        final_unique_count = len(
            set(getattr(ind, "architecture", str(ind.parsed_layers)) for ind in population)
        )
    ...
    def elite_models(self, k_best=1):
        ...
        for i, model in enumerate(topModels):
            self.logger.info(
                f"Selected elite model for next generation. Idx {i} with fitness: {model.fitness}"
            )
        return topModels
    ...
    def evolve(self, mating_pool_cutoff=0.5, mutation_probability=0.85, k_best=1, n_random=3):
        ...
        while len(new_population) < self.n_individuals - n_random - k_best:
            try:
                ...
                assert parent1.parsed_layers != parent2.parsed_layers, (
                    "Parents are the same individual."
                )
            except Exception as e:
            ...
            try:
                ...
                if is_valid:
                    ...
                    assert individual.model_size < self.max_parameters, (
                        f"Model size is {individual.model_size}"
                    )
            ...
            if arch not in unique_architectures:
                ...
                for _ in range(50):
                    try:
                        ...
                        self.logger.warning(f"Failed to replace duplicate individual: {e}")
                        continue
    ...
    def build_model(self, parsed_layers, task="segmentation"):
        ...
        if task == "segmentation":
            ...
            head = MultiInputClassifier(
                shape_tracer(self, encoder.to(self.device)), num_classes=self.dm.num_classes
            )
    ...
    def save_dataframe(self):
        ...
        Returns:
            None
        """
        path = self.save_directory / "src" / f"df_population_{self.generation}.pkl"
        try:
    ...
    def load_dataframe(self, generation):
        path = self.save_directory / "src" / f"df_population_{generation}.pkl"
        try:
    ...
    def save_population(self):
        path = self.save_directory / "src" / f"population_{self.generation}.pkl"
        try:
    ...
    def load_population(self, generation):
        path = self.save_directory / "src" / f"population_{generation}.pkl"
        try:
    ...
    def load_generation(self, generation):
        ...
        population = self.load_population(generation)
        if population is None:
            raise FileNotFoundError(f"Could not load population for generation {generation}")

        self.generation = generation
        self.population = population
        self.df = self.load_dataframe(generation)
        if self.df is None:
            self._update_df()

        self.logger.info(f"Loaded generation {generation} with {len(self.population)} individuals")
    ...
    @staticmethod
    def _first_test_metrics(results):
        ...
        if not isinstance(metrics, dict):
            raise ValueError(f"Expected trainer.test metrics dict, got {type(metrics).__name__}")
        return metrics

    @classmethod
    def _extract_fitness_inputs(cls, results, task):
        metrics = cls._first_test_metrics(results)
        metric_key = "test_iou" if task == "segmentation" else "test_accuracy"
        if metric_key not in metrics:
            raise KeyError(f"Missing required metric '{metric_key}' in test results: {metrics}")
        if "fps" not in metrics:
            raise KeyError(f"Missing required metric 'fps' in test results: {metrics}")

        metric = np.float32(cls._metric_to_float(metrics[metric_key]))
        fps = np.float32(cls._metric_to_float(metrics["fps"]))
        return metric, fps
    ...
    def train_individual(self, idx, task, epochs=20, lr=1e-3, batch_size=None):
        ...
        # ===== Save model =====
        self.save_model(LM, idx=idx, results=results)

        # ===== Test model ===== TODO: Implement a separate test method
        self.logger.info(
            f"[Generation {self.generation} | Individual {idx}] Training completed. Evaluating model..."
        )
        with self._state_lock:
            try:
                #  ===== Extract metrics
                metric, fps = self._extract_fitness_inputs(results, task)

                # ===== Update individual metrics and fitness
                individual.iou = metric
                ...
                self.logger.error(
                    f"[Generation {self.generation} | Individual {idx}] API call failed: {e}"
                )
    ...
    def train_generation(
        ...
        Returns:
            None
        """
        if training_workers is None:
            training_workers = 1
        if not isinstance(training_workers, int) or training_workers < 1:
            raise ValueError(f"training_workers must be a positive integer, got {training_workers}")
        ...
        for idx in range(len(self)):
            if (
                "Fitness" in self.df.columns
                and not pd.isna(self.df.loc[idx, "Fitness"])
                and self.df.loc[idx, "Fitness"] != 0
            ):
        ...
        self.logger.info(
            f"Training generation {self.generation} with {max_workers} parallel workers"
        )
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    self.logger.error(
                        f"[Generation {self.generation} | Individual {idx}] Training failed: {e}"
                    )
                    with self._state_lock:
                        individual = self.population[idx]
                        individual.fitness = None
                        ...
                        if self.df is None or idx not in self.df.index:
                            self._update_df()
                        self.df.loc[idx, "Fitness"] = None
                        self.df.loc[idx, "Metric"] = None
                        self.df.loc[idx, "FPS"] = None
                        self.save_dataframe()
                        self.save_population()
    ...
    def save_model(
        ...
        if idx is None:
            raise ValueError("idx is required to save a model")
        if results is None:
            results = getattr(self, "results", None)
        if results is None:
            raise ValueError("results are required to save a model")

        gen = self.generation
        generation_dir = self.save_directory / f"generation_{gen}"
        generation_dir.mkdir(parents=True, exist_ok=True)

        if ts_save_path is None:
            ts_save_path = generation_dir / f"model_and_architecture_{idx}.pt"
            self.ts_save_path = ts_save_path
        if std_save_path is None:
            std_save_path = generation_dir / f"model_{idx}.pth"
            self.std_save_path = std_save_path
        if openvino_save_path is None:
            openvino_save_path = generation_dir / f"openvino_model_{idx}"

        # Save the results to a text file.
        results_path = generation_dir / f"results_model_{idx}.txt"
        ...
        if save_myriad:
            ...
            temp_onnx_path = Path(tempfile.gettempdir()) / f"temp_model_{idx}.onnx"
            ...
            output_dir = Path(openvino_save_path).resolve()
            ...
            try:
                ...
                xml_path = output_dir / f"temp_model_{idx}.xml"
                from pynas.core.population import (
                    update_config_path,
                )  # Ensure absolute import if needed
