...
class UNetDecoder(nn.Module):
    ...
    def forward(self, encoder_features, verbose=False):
        ...
        if len(encoder_features) != self.num_stages + 1:
            raise ValueError(
                f"Expected {self.num_stages + 1} encoder features, but got {len(encoder_features)}."
            )
...
class GenericUNetNetwork(nn.Module):
    ...
    def _build_encoder(self, parsed_layers, verbose=False):
        ...
        if verbose:
            print(
                f"Initial Input Dimensions: Channels={self.current_channels}, Height={self.current_height}, Width={self.current_width}"
            )
        ...
        try:
            for idx, layer in enumerate(parsed_layers):
                ...
                assert self.current_height > 0 and self.current_width > 0, (
                    f"Invalid output dimensions: height ({self.current_height}) width ({self.current_width}) is zero."
                )
                self.encoder.append(layer_inst)
                self.total_params += sum(p.numel() for p in layer_inst.parameters())
                assert self.total_params <= self.max_params, (
                    f"Exceeded parameter limit. P: {self.total_params:,} > M: {self.max_params:,}"
                )
...
def build_layer(
    layer, config, current_channels, current_height, current_width, idx, get_activation_fn
):
    """
    ...
    if lt in ["ConvAct", "ConvBnAct", "ConvSE"]:
        ...
        assert new_height > 0 and new_width > 0, (
            f"Invalid output dimensions: height ({new_height}) width ({new_width}) is zero."
        )

    elif lt in ["MBConv", "MBConvNoRes"]:
        expansion_factor = int(
            layer.get("expansion_factor", config[lt]["default_expansion_factor"])
        )
        ...
        _, _, _, out_channels, _, _ = parse_conv_params(
            layer, config, lt, current_channels, current_height, current_width
        )
        if lt == "CSPConvBlock":
...
def parse_conv_params(layer, config, key, current_channels, current_height, current_width):
    ...
    out_channels_coeff = float(
        layer.get("out_channels_coefficient", config[key]["default_out_channels_coefficient"])
    )
    out_channels = int(current_channels * out_channels_coeff)
